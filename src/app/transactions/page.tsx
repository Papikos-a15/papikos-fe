...
export default function PaymentsPage() {
    ...
    const filteredTransactions = useMemo(() => {
        return transactions.filter(transaction => {
            ...
            if (dateFromFilter && transactionDate < new Date(dateFromFilter)) return false
            if (dateToFilter) {
    ...
    const hasActiveFilters = typeFilter !== 'all' || statusFilter !== 'all' || dateFromFilter || dateToFilter
        ...
        <main className="flex min-h-screen bg-gradient-to-br from-green-100 via-white to-green-50 items-center justify-center px-4">
            <div className="w-full max-w-6xl p-8 md:p-12 bg-white rounded-xl shadow-xl">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-3xl font-bold text-gray-900">Your Transactions</h2>
                    <Button
                        variant="outline"
                    ...
                    >
                        ...
                        {hasActiveFilters && (
                            <span className="bg-green-600 text-white text-xs rounded-full px-2 py-1 ml-1">
                                {[typeFilter !== 'all', statusFilter !== 'all', dateFromFilter, dateToFilter].filter(Boolean).length}
                            </span>
                        )}
                    </Button>
                </div>
                        ...
                        <table className="w-full table-auto border-collapse border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <tbody>
                                {filteredTransactions.map((transaction, index) => (
                                    <tr key={transaction.id} className={`border-t ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-green-50 transition-colors`}>
                                        <td className="px-4 py-3 font-mono text-sm">{transaction.id.substring(0, 8)}...</td>
                                        <td className="px-4 py-3">
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${transaction.type === 'PAYMENT'
                                                ? 'bg-red-100 text-red-800'
                                                : 'bg-green-100 text-green-800'
                                                }`}>
                                                {transaction.type === 'PAYMENT' ? 'Payment' : 'Top Up'}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${transaction.status.toLowerCase() === 'completed' || transaction.status.toLowerCase() === 'success'
                                                ? 'bg-green-100 text-green-800'
                            ...
                            <div className="flex gap-2">
                                <Button
                                    onClick={async () => {
                                        ...
                                        try {
                                            const res = await fetch("http://localhost:8080/api/transactions/top-up", {
                                                method: "POST",
                                                headers: {
                                                    "Content-Type": "application/json",
                                                    "Authorization": `Bearer ${token}`,
