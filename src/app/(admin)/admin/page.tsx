...
const AdminDashboard = () => {
  ...
  const approveOwner = async (ownerId: string) => {
    ...
    if (res.ok) {
      const updatedOwners = unapprovedOwners.filter((owner: any) => owner.id !== ownerId)
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-green-100 via-white to-green-50 text-gray-800">
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <h1 id="persetujuan-owner" className="text-3xl font-bold mb-6">Dashboard Persetujuan Owner</h1>

        <section className="space-y-4">
          <h2 className="text-xl font-semibold mb-4">Daftar Owner Belum Disetujui</h2>

          {unapprovedOwners.length === 0 ? (
            <p className="text-gray-500">Tidak ada owner yang belum disetujui.</p>
          ) : (
            <table className="w-full border-collapse border border-gray-200 mx-auto text-center">
              <thead>
                <tr className="bg-green-100">
                  <th className="border border-gray-200 px-4 py-2  text-lg">No</th>
                  <th className="border border-gray-200 px-4 py-2  text-lg">Email</th>
                  <th className="border border-gray-200 px-4 py-2  text-lg"></th>
                </tr>
              </thead>
              <tbody>
                {unapprovedOwners.map((owner: any, index: number) => (
                  <tr key={owner.id} className="hover:bg-gray-50">
                    <td className="border border-gray-200 px-4 py-2 text-lg">{index + 1}</td>
                    <td className="border border-gray-200 px-4 py-2 text-lg">{owner.email}</td>
